  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (method.equals("addfiles")) {          // 上传文件
        ...
        int flag = cb.comUp("insert into files(title,url,description,addtime,users) values('" + title + "','" + path + "/" + file.getFileName() + "','" + description + "','" + date2 + "','" + username + "')");
        ...
        count = mySmartUpload.save(path);
        if (count == 0) {
          int flag = cb.comUp("update files set title='" + title + "',description='" + description + "',addtime='" + date2 + "',users='" + username + "' where id='" + id + "'");
          if (flag == Constant.SUCCESS) {
            request.setAttribute("message", "操作成功！");
            request.getRequestDispatcher("admin/manager/manage_file.jsp").forward(request, response);
          } else {
          ...
          int flag = cb.comUp("update files set title='" + title + "',url='" + path + "/" + file.getFileName() + "',description='" + description + "',addtime='" + date2 + "',users='" + username + "' where id='" + id + "' ");
          if (flag == Constant.SUCCESS) {
            request.setAttribute("message", "操作成功！");
            request.getRequestDispatcher("admin/manager/manage_file.jsp").forward(request, response);
          } else {
