...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  if(pageCount==0){	
	  form3.action="<%=basePath%>admin/wj/index.jsp?page=1";
	  form3.submit();
	}
	else{
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
         ...
         <td nowrap="nowrap"><strong>文件名称</strong></td> 
         <td nowrap="nowrap"><strong>下载</strong></td>
         <td nowrap="nowrap"><strong>文件说明</strong></td>
...
<%   
	...
	int cou = cb.getMessageCount("select count(*) from wj where yh='"+username+"'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from wj where yh='"+username+"' order by id desc",7);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap">
         <a href="<%=basePath%>admin/manager/add_file.jsp?method=upwj&id=<%=pagelist2.get(0).toString()%>">修改</a>
         <a href="<%=basePath%>ComServlet?method=delwj&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
