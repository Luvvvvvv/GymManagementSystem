...
<%	String word=Common.toChineseAndTrim(request.getParameter("word")); 
	...
	int cou = cb.getMessageCount("select count(*) from cg where bh like '%"+word+"%' and yh='"+username+"'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from cg where bh like '%"+word+"%' and yh='"+username+"' order by id desc",13);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap">
         <a href="<%=basePath%>admin/coach/add_member.jsp?method=upcg&id=<%=pagelist2.get(0).toString()%>">修改</a>
         <a href="<%=basePath%>ComServlet?method=delcg&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
